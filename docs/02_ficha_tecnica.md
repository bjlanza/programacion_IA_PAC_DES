...
| 4 | Grafana "monitorización" | Grafana con dos dashboards provisioned: Telemetría IoT (InfluxDB) y Estado Pipeline (FastAPI vía Infinity plugin). | ✅ Implementado |
| 5 | `scikit-learn` en stack  | Implementado en `src/api/main.py` como `AnomalyDetector(IsolationForest)`. Singleton en FastAPI. Endpoints: `POST /model/train`, `GET /model/status`, `GET /machines/{id}/predict`. | ✅ Implementado (Aportación A3) |
...
| 10 | Lambda cold path Parquet | `kafka_to_minio.py` (proceso Python) escribe NDJSON particionado en MinIO. DuckDB lo consulta con `read_json()` y `hive_partitioning=true`. El conector FileSystem de Flink fue eliminado por conflictos de JARs. | ✅ Aportación A2 (adaptado) |
| 11 | Streamlit 3 tabs         | Dashboard ampliado a 9 tabs: Tiempo Real, Historial, Alertas, Lambda Query, IA Anomalías, Pipeline Health, Hash Chain, Modelos Avanzados, Ayuda. | ✅ Ampliado |
| 12 | 4 jobs Flink             | 3 jobs Flink activos: `flink_normalization_job`, `flink_analytics_job`, `flink_hash_verifier_job`. El `flink_to_minio_job` fue eliminado (JAR S3/Hadoop incompatible). Cold path cubierto por `kafka_to_minio.py`. | ✅ Adaptado |
